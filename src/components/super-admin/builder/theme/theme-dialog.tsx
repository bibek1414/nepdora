...
} from "@/hooks/super-admin/components/use-theme";
import {
  ThemeData,
  ThemeColors,
  ThemeFonts,
...
import { Palette, Save, Check, Type } from "lucide-react";
...
export const ThemeDialog: React.FC<ThemeDialogProps> = ({
  ...
  const { data: themesResponse, isLoading } = useThemeQuery(templateSlug);
  const createThemeMutation = useCreateThemeMutation(templateSlug);
  const updateThemeMutation = useUpdateThemeMutation(templateSlug);
    ...
    const saveOperation =
      themesResponse?.data && themesResponse.data.length > 0
        ? updateThemeMutation.mutateAsync({
            componentId: themesResponse.data[0].id.toString(),
            data: {
              data: {
                theme: currentTheme,
              },
