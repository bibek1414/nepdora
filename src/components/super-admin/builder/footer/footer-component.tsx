...
import { FooterStyle1 } from "@/components/site-owners/builder/footer/footer-style1";
import { FooterStyle2 } from "@/components/site-owners/builder/footer/footer-style2";
import { FooterStyle3 } from "@/components/site-owners/builder/footer/footer-style3";
import { FooterStyle4 } from "@/components/site-owners/builder/footer/footer-style4";
import { FooterStyle5 } from "@/components/site-owners/builder/footer/footer-style5";
...
export function Footer({
  ...
  const updateFooterMutation = useUpdateFooterMutation(templateSlug);
  const createFooterMutation = useCreateFooterMutation(templateSlug);
  const deleteFooterMutation = useDeleteFooterMutation(templateSlug);
  const { data: existingFooter } = useFooterQuery(templateSlug);
    ...
    try {
      ...
      if (shouldUpdate && footerIdToUse) {
        ...
        const result = await updateFooterMutation.mutateAsync({
          componentId: footerIdToUse,
          data: {
            footerData: processedData,
            // Include other required fields if needed
            ...(existingFooter?.data?.content && {
              content: existingFooter.data.content,
            }),
          },
        });
  ...
  const handleConfirmDelete = () => {
    ...
    const footerIdToDelete =
      existingFooter?.data?.id || existingFooterId || footerId;
    ...
    deleteFooterMutation.mutate(footerIdToDelete);
