...
import { TestimonialCard11 } from "../testimonial-card/testimonial-card-11";
...
export const TestimonialStyle1: React.FC<TestimonialStyleProps> = ({
  data,
  isEditable = false,
  siteUser,
  onUpdate,
        ...
        {!isLoading && !error && testimonials.length > 0 && (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:gap-8 lg:grid-cols-3">
            {testimonials.slice(0, pageSize).map(testimonial => (
              <div
                key={testimonial.id}
                className="relative h-full transition-transform duration-300 hover:-translate-y-1"
              >
