...
const CARD2_SUBTITLE_FALLBACK = "[Team Member]";
...
export const TeamStyle2: React.FC<TeamStyleProps> = ({
  data,
  isEditable = false,
  siteUser,
  onUpdate,
  ...
  const shouldUseCard2SubtitleFallback =
    rawSubtitle === undefined ||
    rawSubtitle === null ||
    rawSubtitle === defaultTeamData.subtitle;

  const resolvedTitle = shouldUseCard2TitleFallback
    ? CARD2_TITLE_FALLBACK
    : title;
  const resolvedSubtitle = shouldUseCard2SubtitleFallback
    ? CARD2_SUBTITLE_FALLBACK
    : subtitle;
