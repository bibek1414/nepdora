...
interface SubCategoryCard4Props {
  ...
  onFeaturedContentUpdate?: (
    updatedData: Partial<NonNullable<SubCategoryData["featuredContent"]>>
  ) => void;
}
...
export const SubCategoryCard4: React.FC<SubCategoryCard4Props> = ({
  ...
  useEffect(() => {
    if (
      !isEditable &&
      featuredContent.backgroundImages &&
  ...
  const currentBackgroundImage =
    featuredContent.backgroundImages?.[featuredContent.currentImageIndex || 0];
    ...
    >
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`h-4 w-4 text-gray-500 transition-transform ${isActive ? "rotate-180" : ""}`}
                      />
                    </button>
            ...
            return (
              <div key={`cat-${category.id}`} className="group relative">
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-lg p-3 text-left text-gray-700 transition-all duration-200 hover:bg-gray-50"
                >
      ...
      <div className="flex flex-1 flex-col">
        ...
        >
          ...
          <div className="container mx-auto px-4 py-12 text-white sm:px-6 md:px-8 md:py-0">
            <EditableText
            ...
            <EditableLink
              ...
              isEditable={isEditable}
              siteUser={siteUser}
              className="inline-flex transform items-center gap-2 rounded-lg px-6 py-3 text-base font-semibold text-white transition-all duration-200 hover:scale-105"
          ...
          {(featuredContent.backgroundImages?.length || 0) > 1 && (
            <>
              <button
                onClick={prevImage}
                className="absolute top-1/2 left-4 z-20 -translate-y-1/2 rounded-full bg-black/30 p-2 text-white transition hover:bg-black/50"
              >
                <ChevronRight className="h-5 w-5 rotate-180" />
              </button>
              <button
                onClick={nextImage}
                className="absolute top-1/2 right-4 z-20 -translate-y-1/2 rounded-full bg-black/30 p-2 text-white transition hover:bg-black/50"
              >
