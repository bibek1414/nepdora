...
interface CountriesComponentProps {
  component: ComponentResponse<"countries">;
  isEditable?: boolean;
  siteUser?: string;
  pageSlug?: string;
  onUpdate?: (
...
export const CountriesComponent: React.FC<CountriesComponentProps> = ({
  ...
  const handleUpdate = (updatedData: Partial<CountriesData>) => {
    if (!pageSlug) return;
  ...
  const renderStyle = () => {
    ...
    switch (style) {
      case "style-1":
      default:
        return <CountriesStyle1 {...commonProps} />;
    }
  };
      ...
      {isEditable && (
        <div className="absolute -right-5 z-30 flex translate-x-full flex-col gap-2 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          >
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Delete Countries Component</AlertDialogTitle>
                <AlertDialogDescription>
                  Are you sure you want to delete this countries component? This
