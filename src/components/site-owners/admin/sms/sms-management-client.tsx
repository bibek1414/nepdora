...
export function SMSManagementClient() {
  ...
  const { data: purchases, isLoading: isPurchasesLoading } =
    useSMSPurchases(purchasePage);
  const { data: history, isLoading: isHistoryLoading } =
    useSMSHistory(historyPage);

  const purchaseItems = purchases?.results || [];
  const historyItems = history?.results || [];

  const PAGE_SIZE = 10;
  const totalPurchasePages = Math.ceil((purchases?.count || 0) / PAGE_SIZE);
  const totalHistoryPages = Math.ceil((history?.count || 0) / PAGE_SIZE);

  return (
    <div className="mx-auto space-y-8 p-4 md:p-8 lg:p-12">
      {/* Header */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="-tight text-3xl font-bold text-slate-900">
            SMS Management
          </h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Card className="-sm transition- hover:-md border-blue-100 bg-blue-50/20">
          ...
          <CardContent>
            ...
            <p className="-wider mt-1 text-[11px] font-medium text-slate-500">
              Credits Available
            </p>
          </CardContent>
        </Card>
            ...
            <div className="">
              <table className="w-full text-left">
                <thead className="border-b border-slate-200 bg-slate-50">
                  <tr>
                    <th className="-wider px-6 py-4 text-xs font-light text-slate-500">
                      Transaction ID
                    </th>
                    <th className="-wider px-6 py-4 text-xs font-light text-slate-500">
                      Credits
          ...
          <Card className="-sm -hidden border-slate-200 text-balance">
            <div className="-x-auto">
              <table className="w-full text-left">
                <thead className="border-b border-slate-200 bg-slate-50">
                  <tr>
                    <th className="-wider px-6 py-4 text-xs font-bold text-slate-500">
                      Receiver
                    </th>
                    <th className="-wider px-6 py-4 text-xs font-bold text-slate-500">
                      Message
