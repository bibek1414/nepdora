      ...
      {selectedCustomer ? (
        ...
        <div className="flex items-center justify-between rounded-lg border border-primary/20 bg-primary/5 px-3 py-2.5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive focus:outline-none"
        ...
        <div className="relative">
          {/* Search input */}
          <div className="relative">
            <Search className="absolute top-1/2 left-3 z-10 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
            <Input
              placeholder="Search by name or phone…"
              value={searchTerm}
              onChange={handleSearchChange}
              className="h-10 border-border bg-muted/40 pl-8 text-sm placeholder:text-muted-foreground focus:bg-background"
            />
            {searchTerm && (
              <button
                onClick={() => { setSearchTerm(""); setIsSearching(false); }}
                className="absolute top-1/2 right-2.5 -translate-y-1/2 cursor-pointer text-muted-foreground hover:text-foreground focus:outline-none"
              >
                <X className="h-3.5 w-3.5" />
              </button>
            )}
          ...
          {/* Dropdown results */}
          {isSearching && (
            <Card className="absolute z-20 mt-1.5 w-full overflow-hidden border border-border p-0 shadow-lg">
                ...
                {isLoading ? (
                  ...
                  <div className="p-1.5">
                    {customersData.results.map(customer => (
                      <button
                        key={customer.id}
                        className="flex w-full cursor-pointer items-center gap-3 rounded-md px-3 py-2 text-left transition-colors hover:bg-accent focus:bg-accent focus:outline-none"
              ...
              <div className="border-t border-border bg-muted/30 p-1.5">
                <button
                  className="flex w-full cursor-pointer items-center justify-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium text-primary transition-colors hover:bg-primary/10 focus:outline-none"
