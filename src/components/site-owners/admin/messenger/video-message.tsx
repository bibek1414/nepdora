...
export function VideoMessage({ videoSrc, thumbnailSrc }: VideoMessageProps) {
  ...
  return (
    <div
      className="relative w-full cursor-pointer overflow-hidden rounded-2xl bg-black shadow-lg"
    ...
    >
      ...
      {isPlaying && <div className="absolute inset-0" onClick={togglePlay} />}

      {/* Bottom Controls Overlay */}
      <div
        className={`absolute right-0 bottom-0 left-0 bg-gradient-to-t from-black/80 to-transparent p-3 transition-opacity duration-300 ${showControls || !isPlaying ? "opacity-100" : "opacity-0"}`}
      >
        {/* Progress Bar */}
        <div
          className="mb-2 h-1 w-full cursor-pointer overflow-hidden rounded-full bg-white/30"
