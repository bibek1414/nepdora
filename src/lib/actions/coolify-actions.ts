...
function getFqdnsFromAppData(appData: any): string {
  if (
    appData.build_pack === "dockercompose" &&
    appData.docker_compose_domains
  ) {
    ...
    if (typeof domainsObj === "string") {
      try {
        ...
        console.error(
          "[Coolify] Failed to parse docker_compose_domains string:",
          e
        );
        return "";
      }
    ...
    if (Array.isArray(domainsObj)) {
      const appService = domainsObj.find(
        (s: any) => s.name === "app" || s.serviceName === "app"
      );
  ...
  try {
    ...
    if (!apiToken || !instanceUrl || !appUuid) {
      ...
      return {
        success: true,
        message:
          "Coolify not configured, skipping optional automated registration.",
      };
    }
    ...
    console.log(
      `[Coolify] Fetching application configuration for ${appUuid}...`
    );
    ...
    if (!response.ok) {
      ...
      console.error(
        `[Coolify] Failed to fetch application: ${response.status} ${errText}`
      );
      return {
        success: false,
        error: `Failed to fetch Coolify application: ${response.status}`,
      };
    }
    ...
    console.log(
      `[Coolify] App Type: ${appData.build_pack}, Current Domains:`,
      currentFqdns
    );

    // Coolify expects FQDNs with protocol (https://)
    const newDomains = [`https://${domainName}`];
    const existingList = currentFqdns
      .split(",")
      .map((s: string) => s.trim())
      .filter(Boolean);
    ...
    if (!changed) {
      console.log(
        `[Coolify] Domain ${domainName} already exists in configuration.`
      );
    ...
    console.log(
      `[Coolify] Updating domains for ${appUuid} to:`,
      fqdnStringToSend
    );
    ...
    if (!updateRes.ok) {
      ...
      if (
        errString.includes("Invalid URL") &&
        updatedList.some(d => d.includes("*"))
      ) {
        console.warn(
          "[Coolify] Wildcard URL detected in failed update. Retry with sanitized list..."
        );
        ...
        const retryRes = await fetch(
          `${baseUrl}/api/v1/applications/${appUuid}`,
          {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${apiToken}`,
        ...
        if (retryRes.ok) {
          return {
            success: true,
            message:
              "Domain added. NOTE: Your existing wildcard (*.nepdora.com) was skipped to bypass a Coolify API conflict.",
          };
        }
      }
      ...
      return {
        success: false,
        error: errData.message || "Failed to update Coolify configuration",
      };
    }
