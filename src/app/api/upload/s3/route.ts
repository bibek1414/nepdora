...
const s3Client = new S3Client({
  ...
  endpoint: process.env.AWS_S3_ENDPOINT_URL || "https://sgp1.digitaloceanspaces.com",
  credentials: {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const customDomain = process.env.NEXT_PUBLIC_AWS_S3_CUSTOM_DOMAIN || `${BUCKET_NAME}.${process.env.AWS_S3_REGION}.digitaloceanspaces.com`;
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to upload to DigitalOcean Spaces" },
      { status: 500 }
    );
  }
}
