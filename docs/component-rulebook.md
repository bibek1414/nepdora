...
- When rendering editable text, prefer the `as` prop look (so `<EditableText as="h3">` instead of wrapping bare `h3` tags) and inject theme tokens through `style` or `className` (colors from `theme.colors.secondary`, fonts from `theme.fonts.heading`) so the UI matches the designer intent.
